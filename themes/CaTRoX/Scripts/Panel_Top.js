...
var barY = 28;
var streamMetadb;
var displayFont = gdi.font("Segoe Ui Semibold", 24, 0);
var menuColor = RGB(35, 35, 45);
var captionColor = RGB(25, 25, 35);
var buttonCount = 0;
var buttonsRight = 0;
var buttonMargin = 0;
createButtonImages();
...
function on_paint(gr) {
    ...
    !test && gr.FillSolidRect(0, 0, ww, wh, captionColor);
    !test && gr.FillSolidRect(4, 4, ww - 8, 24, menuColor);
    //---> Logo
...
function createButtonObjects(ww, wh) {
    ...
    b[5] = new Button(x + (w + p) * ++buttonCount, y, w, h, "Append to current Playlist", btnImg.AppendPlaylist);
    if (showRandomButton) {
        b[6] = new Button(x + (w + p) * ++buttonCount, y, w, h, "Playback/Random", btnImg.PlaybackRandom);
    }
    if (showOpenExplorerButton) {
        b[7] = new Button(x + (w + p) * ++buttonCount, y, w, h, "OpenExplorer", btnImg.OpenExplorer);
    }
...
function createButtonImages() {
    ...
    var fontGuifx = gdi.font("Guifx v2 Transports", 28, 0);
    ...
    for (var i in btn) {
        ...
        for (var s = 0; s <= 2; s++) {
            ...
            //---> 
            if (btn[i].id == "menu") {
                g.FillSolidRect(0, 0, w, h, menuColor);
